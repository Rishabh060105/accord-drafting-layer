...
interface AppState {
  ...
  applyGeneratedDraft: (draft: { templateMarkdown: string; modelCto: string; data: string }) => Promise<void>;
  ...
  immer(
    devtools((set, get) => {
      ...
      return {
        ...
        applyGeneratedDraft: async (draft) => {
          set(() => ({
            templateMarkdown: draft.templateMarkdown,
            editorValue: draft.templateMarkdown,
            modelCto: draft.modelCto,
            editorModelCto: draft.modelCto,
            data: draft.data,
            editorAgreementData: draft.data,
            error: undefined,
          }));

          try {
            const result = await rebuildDeBounce(
              draft.templateMarkdown,
              draft.modelCto,
              draft.data
            );
